...
context('Testar o Login', () => {
 ...
 it('TESTE - Cadastrar Grupo fornecedor', () => {
   
    //seleciona cliente no menu
    cy.get(loc.integracao.icone).click()
    cy.get(':nth-child(1) > :nth-child(2) > ul.ng-scope > :nth-child(2) > .navigation-menu-item > .navigation-menu-button > .title').should('contain','Grupo fornecedor')
    ...
    // //adicioanr
    //cy.contains('CYTESTE A').click()
    cy.get('#btn-adicionar > span.ng-scope').click()
    cy.get('#descricao').clear().type('CYTESTE ').type(faker.name.firstName())
    ...
    cy.get('.toast > .md-icon-button > .ng-scope').click()
    //abrir visualizar
    cy.contains('CYTESTE').dblclick()
    cy.get('[label="Identificação"] > .panel-body').should('contain','CYTESTE')
    cy.get('[ng-include="ctrl.templateLog"] > .panel > .panel-body').should('contain','ATHOS').and('contain', "Últ. Atualização")
    cy.get('[ng-repeat="state in $navigationState.currentState.path"][style=""] > .ng-binding').should('contain', 'Visualizar')
    ...
    cy.get('.toast > .md-icon-button > .ng-scope').click()
    //atualiza
    cy.get('.layout-row > :nth-child(1) > .md-raised').click()
    cy.wait(500)
    cy.get('.form-control').type('TESTE')
    cy.get('.panel').should('contain', 'TESTE')
